...
import { loadSettings, saveSettings, saveAllergies } from '@/lib/storage'
...
export default function OnboardingPage() {
  ...
  const [step, setStep] = useState<'school' | 'allergy'>('school')
  ...
  return (
    <div className="max-w-md mx-auto p-4 pt-10">
      ...
      {step === 'allergy' && (
        <div>
          <h2 className="text-lg font-bold text-gray-900 mb-1">알레르기가 있나요?</h2>
          <p className="text-sm text-gray-400 mb-4">급식 메뉴에서 해당 식품을 강조해드려요</p>
          ...
          <div className="flex gap-2 mt-6">
            <button
              className="flex-1 bg-gray-100 text-gray-500 rounded-xl py-3 text-sm font-semibold"
              onClick={() => router.push('/timetable')}
            >
              건너뛰기
            </button>
            <button
              className="flex-[2] bg-blue-500 text-white rounded-xl py-3 text-sm font-semibold"
              onClick={() => { saveAllergies(selectedAllergies); router.push('/timetable') }}
            >
              시작하기
            </button>
          </div>
